...
interface ServerProxy extends ProxyOptions {
  prefix?: string
  target: string
}

type ServerProxies = Record<string, ServerProxy>

const httpsMatches = /^https:\/\//

export function createProxy(useWs: boolean = true) {
  const serverProxy: Record<string, ProxyOptions> = {}

  const serverProxies: ServerProxies = proxies
