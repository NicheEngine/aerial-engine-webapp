import type { MemoryCache } from 'memory-extend'

export const NOT_ALIVE = 0

export class Memory<T = any, V = any> {
  private cache: { [K in keyof T]?: MemoryCache<V> } = {}
  ...
  setCache(cache: { [K in keyof T]?: MemoryCache<V> }) {
    this.cache = cache
  }
  ...
  resetCache(cache: { [K in keyof T]: MemoryCache<V> }) {
