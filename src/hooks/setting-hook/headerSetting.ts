import type { HeaderSetting } from 'app-settings'

import { computed, unref } from 'vue'

import { useAppStore } from 'app-store'

import { useMenuSettingHook } from './menuSetting.ts'
import { useRootSettingHook } from './rootSetting.ts'
import { useFullContentHook } from '@hks/fullContent.ts'
import { MenuModeType } from '@ems/menuTypes.ts'

export function useHeaderSettingHook() {
  ...
  const getShowInsetHeaderRef = computed(() => {
    const need = !unref(fullContent) && unref(getShowHeader)
    return (
  ...
  const { getShowBreadCrumb, getShowLogo } = useRootSettingHook()
  ...
  const getShowBread = computed(() => {
    return (
      unref(getMenuMode) !== MenuModeType.HORIZONTAL && unref(getShowBreadCrumb) && !unref(getSplit)
    )
  })
...
export default useHeaderSettingHook
