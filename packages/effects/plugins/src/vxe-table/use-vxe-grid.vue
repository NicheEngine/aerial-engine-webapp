...
async function init() {
  ...
  if (formConfig && formConfig.enabled) {
    console.warn(
      '[Engine Vxe Table]: The formConfig in the grid is not supported, please use the `formOptions` props',
    );
  }
  props.api?.setState?.({ gridOptions: defaultGridOptions });
  // form 由 engine-form 代替，所以需要保证query相关事件可以拿到参数
...
watch(
  ...
  () => {
    formApi.setState((prev) => {
      const finalFormOptions: EngineFormProps = mergeWithArrayOverride(
        {},
        formOptions.value,
        prev,
      );
      ...
      <template v-if="showToolbar" #toolbar-actions="slotProps">
        <slot v-if="showTableTitle" name="table-title">
          <div class="mr-1 pl-1 text-[1rem]">
            {{ tableTitle }}
            <EngineHelpTooltip v-if="tableTitleHelp" trigger-class="pb-1">
              {{ tableTitleHelp }}
            </EngineHelpTooltip>
          </div>
        </slot>
