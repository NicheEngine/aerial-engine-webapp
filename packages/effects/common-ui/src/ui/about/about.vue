...
import { EngineRenderContent } from '@aerial-engine-core/shadcn-ui';
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'engine-link' },
    { default: () => text },
  );
...
const engineDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
    content: renderLink(ENGINE_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(ENGINE_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
    content: renderLink(ENGINE_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="text-foreground mt-3 text-sm leading-6">
        <a :href="ENGINE_GITHUB_URL" class="engine-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in engineDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
                <EngineRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
