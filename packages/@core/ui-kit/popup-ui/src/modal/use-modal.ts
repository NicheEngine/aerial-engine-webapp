...
const USER_MODAL_INJECT_KEY = Symbol('ENGINE_MODAL_INJECT');
...
export function useEngineModal<TParentModalProps extends ModalProps = ModalProps>(
...
async function checkProps(api: ExtendedModalApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      ...
      console.warn(
        `[Engine Modal]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Modal, please use useEngineModal or api.`,
      );
    }
  }
}
