...
const ENGINE_APP_CONF = '_ENGINE_APP_CONF_';
  ...
  return {
    ...
    async generateBundle() {
      try {
        ...
        console.log(
          colors.cyan(`✨ configuration file is build successfully!`),
        );
      } catch (error) {
        console.log(
          colors.red(
...
async function getConfigSource() {
  ...
  const windowVariable = `window.${ENGINE_APP_CONF}`;
  // 确保变量不会被修改
  let source = `${windowVariable}=${JSON.stringify(config)};`;
  source += `
    Object.freeze(${windowVariable});
    Object.defineProperty(window, "${ENGINE_APP_CONF}", {
      configurable: false,
      writable: false,
    });
  `.replaceAll(/\s/g, '');
